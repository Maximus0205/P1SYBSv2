...
function DistanceSuggestions({ address, date, orders, onUseDate }) {
  ...
  return (
    <div className="mb-3 rounded-xl border border-success bg-success/10 p-3">
      <p className="text-sm font-semibold text-success flex items-center gap-1.5">
        <MapPin size={14} /> Køreafstand til andre bookinger
      </p>

      {status === "soeger" && <p className="text-xs text-muted mt-1">Tjekker afstand til kommende bookinger...</p>}

      {status === "fejl" && (
        <p className="text-xs text-danger mt-1 flex items-center gap-1.5">
      ...
      {status === "fundet" && (
        <>
          <p className="text-xs text-muted mt-1">Der er allerede planlagte sager tæt på — overvej at samle kørslen:</p>
          <div className="mt-2 space-y-1">
            {suggestions.map((f) => (
              <div key={f.dato} className="rounded-lg bg-white border border-line px-2 py-1.5">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <span className="text-xs font-semibold text-ink">
                    {formatLongDate(f.dato)}
                    <span className="text-success font-semibold"> · ca. {f.km.toFixed(1)} km</span>
                  </span>
                  <button
                    onClick={() => onUseDate(f.dato)}
                    className="text-[10px] font-semibold uppercase tracking-wide text-ink border border-line rounded-full hover:border-success hover:text-success px-2 py-1 shrink-0"
                  >
                    Brug denne dato
                  </button>
                </div>
