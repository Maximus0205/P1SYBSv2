...
function NyeSagForm({ montorer, varetyper, varekategorier, primaerydelser, tillaegsydelser, sager, valgtDato, onAdd, onClose, butikFokus }) {
  ...
  const [date, setDate] = useState(valgtDato || todayISO());
  const [timeSlotId, setTimeSlotId] = useState("heldag");
  const [technicianId, setTechnicianId] = useState("");
  const [lineItems, setLineItems] = useState([createLineItem(varetyper, primaerydelser)]);
  ...
  const addLineItem = () => setLineItems((prev) => [...prev, createLineItem(varetyper, primaerydelser)]);
  ...
  const fillFromPdf = (fields) => {
    ...
    if (fields.varetyper?.length) {
      setLineItems(fields.varetyper.map((navn) => {
        ...
        return createLineItem(varetyper, primaerydelser, vt ? vt.id : undefined, vt ? "" : navn);
      }));
    }
  };
  ...
  return (
    <div className="border border-[#D8D0BE] bg-white p-5">
      ...
      <KvitteringUpload varetyper={varetyper} onUdfyld={fillFromPdf} />
      ...
      <div className="grid gap-3 sm:grid-cols-2 mb-3">
        ...
        <AdresseInput value={address} onChange={setAddress} placeholder="Leveringsadresse" onValideringChange={setAddressStatus} fokus={butikFokus} />
      ...
      <AddressSuggestion address={address} date={date} orders={sager} onUseDate={(d) => setDate(d)} />
      <AfstandsForslag adresse={address} dato={date} sager={sager} onBrugDato={(d) => setDate(d)} />
      ...
      <DailyRouteOverview orders={sager} technicians={montorer} date={date} />
      ...
      <div className="space-y-2 mb-4">
        {lineItems.map((l, idx) => (
          <LineItemEditor key={l.id} lineItem={l} productTypes={varetyper} productCategories={varekategorier} primaryServices={primaerydelser} addOnServices={tillaegsydelser} onChange={(next) => updateLineItem(idx, next)} onRemove={() => removeLineItem(idx)} canRemove={lineItems.length > 1} />
        ))}
      </div>
