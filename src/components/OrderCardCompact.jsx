...
// adresse - til lister hvor pointen er et hurtigt overblik, ikke alle
// detaljer på forhånd. Klik åbner stadig fuld sagsdetalje uanset.
//
// Datoen vises ALTID (kort format, fx "18. aug") - kortet bruges ofte i
...
// given sag lå på uden at åbne den. Er sagen afsluttet, og tidspunktet
// kendt, vises det som en ekstra lille linje.
//
// ÆNDRET (september 2026): STATUSMÆRKET KAN IKKE LÆNGERE KLIKKES.
//
// Det var pakket ind i en <button>, der cyklede status planlagt -> i gang
// -> afsluttet -> planlagt. Da status-skifteren blev fjernet fra resten af
// systemet, holdt de kaldende sider op med at sende onCycleStatus - og så
// ville et klik her kaste en fejl på en undefined funktion. Statusmærket
...
// ingen til kunden i tide.
//
// VIGTIGT om min-w-0: Kortet ligger som CSS grid/flex-element i sine
// forældre. Grid- og flex-elementer har som standard en usynlig
// "min-width: auto", som betyder de IKKE må blive smallere end deres eget
// indhold - typisk et <select>, hvis bredde ellers bestemmes af den
// bredeste <option>-tekst. Uden min-w-0 presser kortet derfor sig selv
// (og alt indhold i det) ud over skærmens kant på mobil, i stedet for at
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        >
          ...
          {mangler.length > 0 && (
            <p className="text-[11px] text-danger font-semibold flex items-center gap-1 mt-0.5">
              <AlertTriangle size={11} className="shrink-0" aria-hidden="true" />
              {mangler.length === 1 ? "En vare kan ikke findes på lageret" : `${mangler.length} varer kan ikke findes på lageret`}
            </p>
          )}
          {!minimal && <p className="text-[10px] text-muted flex items-center gap-1 mt-0.5"><Clock size={10} aria-hidden="true" /> {formatDuration(orderExpectedMinutes(order))} forventet</p>}
          {order.status === "afsluttet" && order.afsluttetTidspunkt && (
            <p className="text-[10px] text-success flex items-center gap-1 mt-0.5"><CalendarCheck2 size={10} aria-hidden="true" /> Afsluttet {formatShortDate(order.afsluttetTidspunkt.slice(0, 10))}</p>
          )}
        </button>
        {/* Ren visning - se noten om status-skifteren ovenfor. */}
      ...
      {(onAssign || onUpdateTimeSlot) && (
        <div className="flex flex-col sm:flex-row gap-2 mt-2 pt-2 border-t border-divider min-w-0">
          {onUpdateTimeSlot && (
            <select value={order.tidsrumId} onChange={(e) => onUpdateTimeSlot(order.id, e.target.value)} aria-label={`Tidsrum for sag ${order.nr}`} className="w-full sm:w-auto min-w-0 rounded-lg text-xs border border-line px-2 py-2 text-muted focus:outline-none focus:border-brand">
