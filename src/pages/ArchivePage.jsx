...
function ArchivePage({ orders, technicians, onOpen }) {
  ...
  return (
    <div>
      ...
      <h1 className="font-['Barlow_Condensed'] text-4xl uppercase tracking-tight text-[#1C232E] mb-1">Arkiv</h1>
      <p className="text-sm text-[#52697E] mb-4">Slå gamle sager op på dato, adresse, telefonnummer eller montør.</p>

      <div className="border border-[#D8D0BE] bg-white p-4 mb-6">
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="text-xs text-[#52697E]">
            ...
            <div className="relative mt-1">
              ...
              <input value={text} onChange={(e) => changeAndReset(setText)(e.target.value)} placeholder="Fx kundenavn eller sagsnr." className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] focus:outline-none focus:border-[#E2621B]" />
            </div>
          </label>
          <label className="text-xs text-[#52697E]">
            Adresse
            <div className="relative mt-1">
              <MapPin size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#52697E]" />
              <input value={address} onChange={(e) => changeAndReset(setAddress)(e.target.value)} placeholder="Fx gadenavn eller by" className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] focus:outline-none focus:border-[#E2621B]" />
            </div>
          </label>
          <label className="text-xs text-[#52697E]">
            Telefonnummer
            <div className="relative mt-1">
              <Phone size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#52697E]" />
              <input value={phone} onChange={(e) => changeAndReset(setPhone)(e.target.value)} placeholder="Fx 12345678" className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] focus:outline-none focus:border-[#E2621B]" />
            </div>
          </label>
          <label className="text-xs text-[#52697E]">
            Montør
            <div className="relative mt-1">
              <User size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#52697E] pointer-events-none" />
              <select value={technicianId} onChange={(e) => changeAndReset(setTechnicianId)(e.target.value)} className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] focus:outline-none focus:border-[#E2621B]">
          ...
          <label className="text-xs text-[#52697E]">
            Fra dato
            <div className="relative mt-1">
              <Calendar size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#52697E] pointer-events-none" />
              <input type="date" value={fromDate} onChange={(e) => changeAndReset(setFromDate)(e.target.value)} className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] font-mono focus:outline-none focus:border-[#E2621B]" />
            </div>
          </label>
          <label className="text-xs text-[#52697E]">
            Til dato
            <div className="relative mt-1">
              <Calendar size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#52697E] pointer-events-none" />
              <input type="date" value={toDate} onChange={(e) => changeAndReset(setToDate)(e.target.value)} className="w-full border border-[#D8D0BE] bg-[#F3EFE6] pl-8 pr-3 py-2 text-sm text-[#1C232E] font-mono focus:outline-none focus:border-[#E2621B]" />
            </div>
          </label>
        </div>
        {hasFilter && (
          <button onClick={clearAll} className="mt-3 text-xs font-semibold uppercase tracking-wide text-[#52697E] hover:text-[#E2621B] flex items-center gap-1">
      ...
      {!hasFilter ? (
        <p className="text-sm text-[#52697E] italic">Angiv mindst ét søgekriterie ovenfor for at slå sager op.</p>
      ) : (
        <div>
          <p className="text-xs text-[#52697E] mb-3">{results.length} {results.length === 1 ? "sag" : "sager"} matcher, nyeste først</p>
          {results.length === 0 ? (
